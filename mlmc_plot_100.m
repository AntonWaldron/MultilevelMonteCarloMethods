...
if novalue
    ...
    % Plots pairs (eps, sqrt(rms + ), (eps, rms/eps), (eps, abs(ave/eps))
    % etc.
  ...
  % MC error - ( eps, rms/eps)
  % weak error - ( eps, abs(ave./eps)
  % ave = sum(err,2)/100;
  % rms = sqrt((sum(err.^2,2)/100 - ave.^2)); %
  % err = data - value

  % ave = sum(error^2)/100
  % rms = sqrt((sum(err.^2)/100 - ave.^2))
  % weak error is absolute value of average / epsilon. 
end

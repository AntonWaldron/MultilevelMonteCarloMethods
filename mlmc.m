...
%
  ...
  Nl(1:L+1)       = 0;
  suml(1:2,1:L+1) = 0; % 2 x L + 1 matrix initialised to 0. Row 1: Y, Row 2: Y^2
  costl(1:L+1)    = 0; % Cost vector of length L+1 initiliased to 0, cost for each level
  dNl(1:L+1)      = N0; % Num. Samples vector of length L + 1 initialised to 0

  while sum(dNl) > 0 % While dNl = max(0, Ns - Nl) > 0 where Ns is the optimal number of samples

%
% update sample sums
%   % Iterates through the levels, updating values. 
    for l=0:L
      if dNl(l+1) > 0 %dnL = max(0, Ns-Nl) where Ns is the optimal number of samples
        [sums, cost] = mlmc_l(l,dNl(l+1), varargin{:}); % Calling the opre_l function while dNl < Ns from level 0 to level L
        Nl(l+1)     = Nl(l+1)     + dNl(l+1); % Updating the number of samples used
        suml(1,l+1) = suml(1,l+1) + sums(1); % sum(1) = sums(1) + (Pf-Pc); % Becomes E[P_{l} - P_{f}] once divided by Nl
        suml(2,l+1) = suml(2,l+1) + sums(2); % sums(2) = sums(2) + ((Pf-Pc).^2);
        costl(l+1)  = costl(l+1)  + cost; % Updating the cost successively
      end
    end
...
%
    ml = abs(   suml(1,:)./Nl); % Abs of E[Pf-Pc]
    Vl = max(0, suml(2,:)./Nl - ml.^2); % Variance calculation:  Y^{2}/Number of samples - E[X]^{2} where E[X] = Y
    Cl = costl./Nl; % Cost across levels. 
...
%
    ...
    % From MLMC theory optimal N value is Ns
    Ns  = ceil( sqrt(Vl./Cl)*sum(sqrt(Vl.*Cl)) / ((1-theta)*eps^2) );
    
    % If 0, while loop ends.
    dNl = max(0, Ns-Nl); 
...
%
    ...
    end % End of loop for checking if additional level is required. 
...
%
  P = sum(suml(1,:)./Nl); %
  % Sum( Y1 / N1 + Y2/N2 + ... etc.?) sum of differences. 
